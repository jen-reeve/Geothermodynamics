...
temp<-seq(500+273.15,5000+273.15,by=100) # in K
...
a<-deltaH+temp*deltaS+
  deltaa*temp+0.5*deltab*temp^2-1*deltac*temp^-1+2*deltad*temp^0.5-
  temp*(deltaa*log(temp)+deltab*temp-0.5*deltac*temp^-2-2*deltad*temp^-0.5)
b<-deltaV+deltathermal*(temp-init_temp)-deltacompress*init_press
c<--0.5*deltacompress
#solving the polynomial
press_plus<-(-b+sqrt(b^2-4*a*c))/2*a # in kbar
press_minus<-(-b-sqrt(b^2-4*a*c))/2*a #these are all negatives so don't actually matter here
...
ggplot(tp,aes(x=temp,y=press_plus))+geom_point()+labs(x="Temperature (K)",y="Pressure (kbar)")
```
